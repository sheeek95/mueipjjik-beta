...
- 옷차림 태그 목록 위에 오늘 코디의 색 조합을 미리 보여주는 컬러 스와치 줄이 따로 떠요(옷 태그 안에 아이콘을 같이 넣으면 UI가 어색해서 별도 줄로 분리함). 각 스와치는 카테고리별 실루엣 아이콘(티셔츠/니트/원피스/코트/청바지 등 약 20종, `ICON_SVGS`)이 그 조합에 배정된 색(`STYLE_COLOR_BANK`) 배경 위에 흰색으로 올라간 형태예요. 200개 조합(8개 기온대 × 5개 스타일 × 5개 대안) 각각이 저마다 다른 색 조합을 갖도록 공식으로 계산해요 — 200개를 일일이 손으로 지정하는 대신, 스타일·계절·순번 조합마다 팔레트에서 다른 색을 골라오는 방식이에요.
