...
## 이번 버전에서 바뀐 점
- **AI 피팅**: "준비 중" 잠금 화면으로 변경 (베타에서는 코디 상담만 제공)
- **설정 페이지**: 내 위치 / 약관 및 정책만 노출 (알림·문의·계정 관리는 다음 버전에서)
- **Claude API 키 보호**: 브라우저가 Claude API를 직접 호출하지 않고, `functions/api/chat.js` 서버리스 함수를 거쳐요. 키는 서버 환경변수에만 있고 브라우저에는 절대 노출되지 않아요.
