...
- "핏치의 한마디"는 체감온도 구간(폭염/더움/무난/쌀쌀/한파)별로 여러 문구 중 하나를 매번 랜덤으로 골라서 보여줘서 매번 똑같은 말이 안 나와요. 또 지금 당장은 비가 안 와도 확인 시점 이후 8시간 안에 강수확률이 50% 이상으로 오르는 시간대가 있으면 "몇 시쯤부터 비 소식이 있으니 우산 챙겨"처럼 구체적인 시간을 짚어서 알려줘요(알림 탭에도 동일하게 반영).
- 기온·습도로 **불쾌지수**(Thom 공식, 기상청이 쓰는 섭씨 버전)를 계산해서 75 이상(높음)이면 옷차림과 별개로 "야외활동 자제"·"수분 섭취" 같은 행동 조언을 핏치의 한마디와 알림에 추가로 얹어줘요. 옷 추천에만 머무르지 않고 날씨 자체(더위/습함/비/눈)에 대해서도 이야기하도록 확장한 거예요.
